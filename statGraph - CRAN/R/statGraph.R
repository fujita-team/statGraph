...
#'
#' A string that indicates one of the following models: "ER" (Erdős–Rényi random
...
#'
#' \code{graph.param.estimator} estimates the parameter that best approximates the
...
#'
#' 0 to 1 with step 'eps' for the "ER" model (Erdős–Rényi random graph), in
...
#'
#' A vector of strings cotaining some of the following models: "ER" (Erdős–Rényi
...
#' (Erdős–Rényi random graph model), "GRG" (geometric random graph model), "WS"
...
#'
#' The function \code{graph.acf} computes estimates of the autocorrelation function
#' for graphs.
#'
...
#'
#' \code{graph.mult.scaling} performs multidimensional scaling of graphs. It takes
...
#'
#' Given two independent finite-dimensional random dot product graphs, \code{tang}
...
#'
#' Given two lists of graphs generated by the inhomogeneous random graph model, \code{ghoshdastidar} tests if they were generated by the same parameters.
#'
...
#'
#' \code{fast.eigenvalue.probability} returns the probability of an eigenvalue given the degree and excess degree probability.
#'
...
#'
#' @param z  Complex number whose real part is the eigenvalue whose probability we want to obtain, the imaginary part is a small value (e.g., 1e-3).
#'
...
#'
#' \code{fast.spectral.density} returns the degree-based spectral density in the the interval \[from,to\] by using npoints discretization points.
#'
#' @param graph The undirected unweighted graph (igraph type) whose spectral density we want to obtain.
#'
#' @param from Lower end of the interval that contain the eigenvalues or smallest eigenvalue of the adjacency matrix of the graph. The smallest eigenvalue is used if the value is not given.
#'
#' @param to  Upper end of the interval that contain the eigenvalues or largest eigenvalue of the adjacency matrix of the graph. The largest eigenvalue is used if the value is not given.
#'
...
#' density <- fast.spectral.density(graph = G, npoints = 10, numCores = 1)
#' density
#'
#' @export
fast.spectral.density <- function(graph, from = NULL, to = NULL,npoints = 2000,numCores = 1){
  `%dopar%` <- foreach::`%dopar%`
  # Number of vertices
...
#'
#' \code{fast.graph.param.estimator} estimates the parameter of the complex network model using the degree-based spectral density and ternary search.
#'
...
#'
#' A string that indicates one of the following models: "ER" (Erdős–Rényi random
#' graph model), "GRG" (geometric random graph model), "WS" (Watts-Strogatz model), and "BA" (Barabási-Albert model).
#'
...
#'
#' @param from Lower end of the interval that contain the eigenvalues to generate the degree-based spectral densities. The smallest eigenvalue of the adjacency matrix corresponding to ``graph'' is used if the value is not given.
#'
#' @param to  Upper end of the interval that contain the eigenvalues to generate the degree-based spectral densities. The largest eigenvalue of the adjacency matrix corresponding to ``graph'' is used if the value is not given.
#'
...
#' estimated.parameter <- fast.graph.param.estimator(G, "WS", npoints = 10,
#'                                                   numCores = 1)
#' estimated.parameter
...
#' }
#' # Obtain the parameter of the WS model
#' estimated.parameter <- fast.graph.param.estimator(G, fun_WS, lo = 0, hi = 1,
#'                                                   npoints = 10, numCores = 1)
#' estimated.parameter
#'
#' @export
fast.graph.param.estimator <- function(graph, model, lo = NULL, hi = NULL, eps = 1e-3, from = NULL,to = NULL,npoints = 2000,numCores = 1){
  ...
  observed_graph_density <- fast.spectral.density(graph, from = from, to = to,npoints = npoints,numCores = numCores)
  # Make ternary search
  dist_to_1 <- NULL
  dist_to_2 <- NULL
...
#' models: "ER" (Erdős–Rényi random graph), "GRG" (geometric random graph), "KR"
...
#'
#' @references
#' Celeux, Gilles, and Gérard Govaert. "Gaussian parsimonious clustering

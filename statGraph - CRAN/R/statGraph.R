...
#'
#' 'GIC' returns the Kullback-Leibler divergence or L2 distance between an
...
#'
#' @examples
#' A <- as.matrix(igraph::get.adjacency(igraph::sample_gnp(n=100, p=0.5)))
...
#'
#' 'eigenvalue.degree.based.probability' returns the probability of an eigenvalue given the degree and excess degree probability.
#'
#' @param deg_prob The degree probability of the graph.
#'
#' @param q_prob The excess degree probability of the graph.
#'
#' @param all_k  List of sorted unique degrees that are greater than 1.
#'
#' @param z  A complex number whose real part is the eigenvalue whose probability we want to obtain, the imaginary part is a small value (e.g., 1e-3).
#'
#' @param n_iter The maximum number of iterations to perform.
#'
#' @return A complex number whose imaginary part absolute value corresponds to the probability of the given eigenvalue.
#'
...
#' # Obtain the probability of finding an eigenvalue 0
#' z <- 0 + 0.01*1i
#' eigenval_prob <- -Im(eigenvalue.degree.based.probability(deg_prob,q_prob,all_k,z))
#' eigenval_prob
#'
#' @export
eigenvalue.degree.based.probability <- function(deg_prob,q_prob,all_k,z,n_iter = 5000){
  h_z   <- 0 + 0i
  eps <- 1e-7
  all_k_mo <- all_k - 1
  while(n_iter > 0){
...
#'
#' 'degree.based.spectral.density' returns the degree-based spectral density in the interval '[from, to]' by using 'npoints' discretization points.
#'
#' @param Graph The undirected unweighted graph whose spectral density we want to obtain.
#'
#' @param from Lower end of the interval that contains the eigenvalues or smallest eigenvalue of the adjacency matrix of the graph. If this parameter value is null, then the smallest eigenvalue is used.
#'
#' @param to  Upper end of the interval that contains the eigenvalues or largest eigenvalue of the adjacency matrix of the graph. If this parameter values is null, then the largest eigenvalue is used.
#'
#' @param npoints The number of discretization points of the interval '[from, to]'.
#'
#' @param numCores Number of cores to use for parallelization.
#'
#' @return Returns the degree-based spectral density of the graph in the interval '[from, to]' discretized using 'npoints'.
#'
...
#' density <- degree.based.spectral.density(Graph = G, npoints = 10, numCores = 1)
#' density
#'
#' @export
degree.based.spectral.density <- function(Graph, from = NULL, to = NULL,npoints = 2000,numCores = 1){
  # Number of vertices
  ...
  y <- foreach::foreach(i=1:length(x),.combine = c) %dopar% {
    z <- x[i] + 0.01*1i
    -Im(eigenvalue.degree.based.probability(deg_prob,q_prob,all_k,z))
  }
...
#'
#' 'degree.based.graph.param.estimator' estimates the parameter that best approximates the model to the observed graph by using the degree-based spectral density and ternary search.
#'
#' @param Graph The undirected unweighted graph.
#'
#' @param model Either a string or a function:
#'
#' A string that indicates one of the following models: "ER" (Erdos-Renyi random
#' graph), "GRG" (geometric random graph), "WS" (Watts-Strogatz model), and "BA" (Barabasi-Albert model).
#'
#' A function that returns a Graph generated by a graph model. It must contain two arguments: the first one
#' corresponds to the graph size and the second to the parameter of the model.
#'
#' @param lo The smallest parameter value that the graph model can take.
#'
#' If 'model' is a string, then the default value of '0' is used for the predefined models ("ER", "GRG", "WS", and "BA").
#'
#' @param hi The largest parameter value that the graph model can take.
#'
...
#'
#' @param from Lower end of the interval that contain the eigenvalues to generate the degree-based spectral densities. If this value is null, the smallest eigenvalue of the adjacency matrix corresponding to 'Graph' is then used.
#'
#' @param to  Upper end of the interval that contain the eigenvalues to generate the degree-based spectral densities. If this value is null, the largest eigenvalue of the adjacency matrix corresponding to 'Graph' is then used.
#'
#' @param npoints The number of points to discretize the interval '[from, to]'.
#'
#' @param numCores The number of cores to use for parallelization.
#'
#'
#' @return Returns a list containing:
#' \item{param}{The degree-based parameter estimate. For the "ER", "GRG", "WS", and "BA"
#' models, the parameter corresponds to the probability to connect a pair of
#' vertices, the radius used to construct the geometric graph in a unit square,
#' the probability to reconnect a vertex, and the scaling exponent respectively.}
#' \item{dist}{The L1 distance between the observed graph and the graph model with the estimated value.}
#'
...
#' estimated.parameter <- degree.based.graph.param.estimator(G,"WS",npoints = 10,numCores = 1)
#' estimated.parameter
...
#' }
#' # Obtain the parameter of the WS model
#' estimated.parameter <- degree.based.graph.param.estimator(G, fun_WS, lo = 0, hi = 1, npoints = 10, numCores = 1)
#' estimated.parameter
#'
#' @export
degree.based.graph.param.estimator <- function(Graph, model, lo = NULL, hi = NULL, eps = 1e-3, from = NULL,to = NULL,npoints = 2000,numCores = 1){

  # When the model is a function then check if the smallest and largest values that the parameter can take was also provided
  ...
  observed_graph_density <- degree.based.spectral.density(Graph, from = from, to = to,npoints = npoints,numCores = numCores)
  # Make ternary search
  dist_to_1 <- NULL
  dist_to_2 <- NULL
  while(abs(lo - hi) > eps && lo < hi){
    ...
    density1 <- degree.based.spectral.density(Graph1, from = from, to = to,npoints = npoints,numCores = numCores)
    ...
    density2 <- degree.based.spectral.density(Graph2, from = from, to = to,npoints = npoints,numCores = numCores)
...
#'
#' 'gCEM' clusters graphs following an expectation-maximization algorithm based
...
#'
#' @references
#' Celeux, Gilles, and G?rard Govaert. "Gaussian parsimonious clustering
#' models." Pattern recognition 28.5 (1995): 781-793.
#'
#' @examples
#' g <- list()
#' for(i in 1:3){
#'   g[[i]] <- igraph::get.adjacency(igraph::sample_smallworld(dim=1, size=30, nei=2 ,p=0))
#' }
#' for(i in 4:6){
#'   g[[i]] <- igraph::get.adjacency(igraph::sample_smallworld(dim=1, size=30, nei=2 ,p=1))
#' }
#' res <- gCEM(g, model="WS", num_clusters=2, ncores=2)
#' res
#'
#' @export
gCEM <- function(g, model, num_clusters, ncores=1){
  `%dopar%` <- foreach::`%dopar%`
  `%:%` <- foreach::`%:%`
...
#' K-means for Graphs
#'
#' 'kmeans.graph' clusters graphs following a k-means algorithm based on the
...
#' g <- list()
#' for(i in 1:5){
#'   g[[i]] <- igraph::get.adjacency(igraph::sample_gnp(30, p=0.2))
#' }
#' for(i in 6:10){
#'   g[[i]] <- igraph::get.adjacency(igraph::sample_gnp(30, p=0.5))
#' }

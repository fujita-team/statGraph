...
#'
#' \code{graph.spectral.density} returns the spectral density computed with either the exact
#' or degree-based method in the interval <\code{from},\code{to}> by using npoints discretization points.
#'
#' @param Graph (un)weighted undirected graph or the adjacency matrix. If the
#' input is a graph then the weights of the edges should be stored in the
#' "weight" attribute of the edge. 
#'
...
#'
#' @param method The method to use to obtain the spectral density. If 'diag' (Default) then the exact
#' spectral density will be obtained, if 'fast' then the degree-based spectral density will be obtained.
#' 
#' @param ... Other relevant parameters for \code{graph.diag.spectral.density'} or \code{graph.fast.spectral.density}.
#'
#' @return Returns the exact spectral density of the graph
#'
...
#' # Obtain the exact spectral density
#' density <- graph.spectral.density(Graph = G, npoints = 80,method = "fast")
#' density
#'
#' @export
#' 
graph.spectral.density <- function(Graph, from = NULL, to = NULL, npoints = 1024,method = "diag",...){
  parameters = list(Graph = Graph,from = from,to = to,npoints = npoints)
  other_parameters = list(...)
  all_parameters = c(parameters,other_parameters)
  
  den_fun <- match.fun(paste0("graph.",method,".spectral.density"))
  
  return (do.call(den_fun,all_parameters))
}


#' Exact Graph spectral density
#'
#' \code{graph.diag.spectral.density} returns the exact spectral density in
...
#' "bcv" is an abbreviation of biased cross-validation.
#'
#' @param eigenvalues optional parameter. It contains the eigenvalues of the
#' adjacency matrix of Graph. Then, if the eigenvalues of Graph have already
#' been computed, this parameter can be used instead of Graph. If no value is passed, then the
#' eigenvalues of Graph will be computed by 'graph_entropy'.
#' 
#' @param method The method to use to obtain the spectral density. If 'diag' (Default) then the exact
#' spectral density will be obtained, if 'fast' then the degree-based spectral density will be obtained.
#' If parameter 'eigenvalues' is not NULL then the exact spectral density is obtained.
#' 
#' @param ... Other relevant parameters for \code{graph.spectral.density}.
#'
...
#'
#' @export
graph.entropy <- function(Graph=NULL, eigenvalues=NULL,method = "diag",...) {
  # compute the entropy
  if (is.null(eigenvalues))
    f <- graph.spectral.density(Graph, method = method,...)
  else
    parameters <- list(...)
...
#' the argument 'p'.
#'
#' A function that returns a graph (or its adjacency matrix)
#' generated by a graph model  at the same points . It must contain 
#' two arguments: the first one corresponds to the graph size and the second 
#' to the parameter of the model. 
...
#' "bcv" is an abbreviation of biased cross-validation.
#'
#' @param eigenvalues optional parameter. It contains the eigenvalues of Graph.
#' Then, it can be used when the eigenvalues of Graph were previously computed.
#' If no value is passed, then the eigenvalues of Graph will be computed by 'GIC'.
#'
...
#'
#' @examples
#' Graph <- igraph::sample_gnp(n=50, p=0.5)
#' # Using a string to indicate the graph model
#' result1 <- GIC(Graph, "ER", 0.5)
#' result1
#'
...
#' result2
#' @export
GIC <- function(Graph, model, p=NULL, eigenvalues=NULL, dist = "KL") {
  if (is.null(eigenvalues)){
    eigenvalues <- graph.eigen(Graph)
  }
  #
  if (methods::is(model,"list")) {
    ...
    f2 <- graph.model.spectral.density(fun, ncol(A), p, from=min(eigenvalues),
                                       to=max(eigenvalues), method = method,...)
    if (sum(is.na(f2)) > 0)
      return(Inf)
    f1 <- gaussianDensity(eigenvalues, from=min(f2$x), to=max(f2$x),
                          bandwidth=bandwidth, npoints=1024)
...
#'
#' \code{graph.param.estimator} estimates the parameter by using either
#' the exact spectral densities or the degree-based spectral densities
#'
#' @param Graph (un)weighted undirected graph or the adjacency matrix. If the
#' input is a graph then the weights of the edges should be stored in the
#' "weight" attribute of the edge. 
...
#'
#' @param method The method to use to estimate the graph parameter. If 'diag' (Default) then the
#' parameter estimator that uses the exact spectral densities is used, if 'fast' then the parameter 
#' estimator that uses the degree-based spectral densities is used.
#' 
#' @param ... Other relevant parameters for \code{graph.diag.param.estimator} or \code{graph.fast.param.estimator}.
#'
...
#' @examples
#' require(igraph)
#' Graph <- erdos.renyi.game(50, p=0.5)
#'
#' # Using a string to indicate the graph model
#' result1 <- graph.param.estimator(Graph, "ER", eps=0.25)
#' result1
#'
#' ## Using a function to describe the graph model
#' ## Erdos-Renyi graph
#' # model <- function(n, p) {
#' #    return(erdos.renyi.game(n, p))
#' # }
#' # result2 <- graph.param.estimator(Graph, model, method = "diag", parameters = seq(0.2, 0.8, 0.1))
#' # result2
#' # result3 <- graph.param.estimator(Graph, model, method = "fast", parameters = list("lo"=0.1,"hi" = 0.2))
#' # result3
#' @export
graph.param.estimator <- function(Graph,model,method = "diag",...){
  parameters = list(Graph = Graph,model = model)
  other_parameters = list(...)
  all_parameters = c(parameters,other_parameters)
  
  den_fun <- match.fun(paste0("graph.",method,".param.estimator"))
...
#' Graph parameter estimator with extact spectral densities
#'
#' \code{graph.diag.param.estimator} estimates the parameter that best approximates
...
#' considered for the parameter estimation (only when search is equal to 'grid'), or 
#' a list with elements 'lo' and 'hi' defining an interval search 
#' of the parameter. 
#' The 'graph.diag.param.estimator' will
...
#'
#' and 0 to 5 with step 'eps' for the "BA" model (Barabasi-Albert model), in
#' which the parameter corresponds to the scaling exponent.
#'
#' @param eps precision of ternary search or of the grid (default is 0.01).
#'
...
#' @param eigenvalues optional parameter. It contains the eigenvalues of Graph.
#' Then, it can be used when the eigenvalues of Graph were previously computed.
#' If no value is passed, then the eigenvalues of Graph will be computed by
#' 'graph_param_estimator'.
#'
#' @param search string indicating the method of searching the parameter. If 
#' "ternary" (default) the parameter is estimated using ternary search. 
#' If "grid" the parameter is estimated using grid search.
#'
#' @return A list containing:
#' \item{p}{the parameter estimate. For the "ER", "GRG", "KR", "WS", and "BA"
#' models, the parameter corresponds to the probability to connect a pair of
...
#'
#' # Using a string to indicate the graph model
#' result1 <- graph.diag.param.estimator(Graph, "ER", eps=0.25)
#' result1
#'
#' ## Using a function to describe the graph model
#' ## Erdos-Renyi graph
...
#' #    return(erdos.renyi.game(n, p))
#' # }
#' # result2 <- graph.diag.param.estimator(Graph, model,  seq(0.2, 0.8, 0.1))
#' # result2
#' @export
graph.diag.param.estimator <- function(Graph, model, parameters=NULL, eps=0.01,
                                       bandwidth="Silverman", eigenvalues=NULL,
                                       spectra = NULL, search = "ternary") {
  #
  if (!methods::is(model,"function") && is.null(parameters)) {
    stop("It is necessary to enter the parameters that will be evaluated.")
  }
  
  if(methods::is(model,"function") && search == "ternary"){
    warning("The ternary search only works for 'model' = \"ER\",\"GRG\",\"KR\",\"BA\" or \"WS\".\nChanging for search = 'grid'.")
    classic = "grid"
  }
  ...
  if(classic == "grid")
  {
    if (!is.null(spectra)) {
      ...
      else if (model == "BA") parameters <- seq(5,0.1, -eps)
      else if (model == "KR") parameters <- as.integer(seq(0, 1, eps)*n)
      else parameters <- seq(1, 0.1, -eps)
    } else if(methods::is(parameters,"list")) {
      parameters <- seq(parameters$hi,parameters$lo,-eps)
    }
    pmin <- -1
    klmin <- Inf
    y = c()
    for (p in parameters){
      ...
      else kl <- GIC(Graph, model, p, bandwidth, eigenvalues = eigenvalues)
      y = append(y,kl)
      if (kl < klmin) {
        klmin <- kl
        pmin <- p
    ...
    p = sum(A)/(n*(n-1))
    kl <- GIC(Graph, model, p, bandwidth, eigenvalues=eigenvalues)
    out <- list("p" = p, "GIC" = kl)
  }
    ...
    for(interval in intervals)
    {
      ...
      while(TRUE)
      {
        if(abs(lo - hi) < eps || lo > hi)
        {
          ...
          if (!is.null(spectra)) klmin <- GIC(Graph, spectra[as.character(pmin),,], pmin, bandwidth, eigenvalues=eigenvalues)
          else klmin <- GIC(Graph, model, pmin, bandwidth, eigenvalues=eigenvalues)
          break
        }
        ...
        if (!is.null(spectra)){
          cost1 <- GIC(Graph, spectra[as.character(leftThird),,], leftThird, bandwidth, eigenvalues=eigenvalues)
          cost2 <- GIC(Graph, spectra[as.character(rightThird),,], rightThird, bandwidth, eigenvalues=eigenvalues)
        }
        else{
          cost1 <- GIC(Graph, model, leftThird, bandwidth, eigenvalues=eigenvalues)
          cost2 <- GIC(Graph, model, rightThird, bandwidth, eigenvalues=eigenvalues)
        }
...
#'
#' 0 to 1 with step 'eps' for the "WS" model (Watts-Strogatz model), in which
#' the parameter corresponds tmodelso the probability to reconnect a vertex;
#'
...
#'
#' require(igraph)
#' Graph <- erdos.renyi.game(30, p=0.5)
#' # Using strings to indicate the graph models
#' result1 <- graph.model.selection(Graph, models=c("ER", "WS"), eps=0.5)
#' result1
...
#'     return(watts.strogatz.game(1, n, K, pr))
#' }
#' parameters <- list(seq(0, 1, 0.5), seq(0, 1, 0.5))
#' result2 <- graph.model.selection(Graph, list(model1, model2), parameters)
#' result2
#' @export
graph.model.selection <- function(Graph, models=NULL, parameters=NULL, eps=0.01,
                                  bandwidth="Silverman", eigenvalues=NULL) {
  A <- graphToAdj(Graph)
  n <- ncol(A)
  ...
  for (i in 1:length(models)) {
    if (!is.null(parameters))
      p <- parameters[[i]]
    r <- graph.diag.param.estimator(A, models[[i]], p, eps, bandwidth,
...
#' graphs.
#'
#' Given two lists of graphs, 'Graphs1' and 'Graphs2', 'graph.test' tests H0: "JS divergence
#' between 'Graphs1' and 'Graphs2' is 0" against H1: "JS divergence between 'Graphs1' and 'Graphs2' is
#' larger than 0".
#'
#' @param Graphs1 a list of (un)weighted undirected graph or the adjacency matrix. If the
#' input is a list of weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge.  
#'
#' @param Graphs2 a list of (un)weighted undirected graph or the adjacency matrix. If the
#' input is a list of weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge. 
#'
...
#' for (i in 1:20)
#'    Graphs1[[i]] <- erdos.renyi.game(50, p=0.5)
#' for (i in 1:20)
#'    Graphs2[[i]] <- erdos.renyi.game(50, p=0.51)
#'
#' result <- graph.test(Graphs1, Graphs2, numBoot=100)
#' result
#'
#' @export
graph.test <- function(Graphs1, Graphs2, numBoot=1000, bandwidth="Silverman") {
  Graphs <- append(Graphs1, Graphs2)
...
#' function.
#'
#' @param Graphs a list of list of (un)weighted undirected graphs or their adjacency matrix. If the
#' lists contain weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge. 
#'
...
#' g1 <- g2 <- g3 <- list()
#' for (i in 1:20) {
#'    g1[[i]] <- erdos.renyi.game(50, 0.50)
#'    g2[[i]] <- erdos.renyi.game(50, 0.50)
#'    g3[[i]] <- erdos.renyi.game(50, 0.52)
#' }
#' Graphs <- c(g1, g2, g3)
#' label <- c(rep(1,20),rep(2,20),rep(3,20))
#' result <- anogva(Graphs, label, numBoot=50)
#' result
#'
#' @references
#'
...
#'
#' @param Graphs a list of (un)weighted graphs or a list of adjacency (symmetric) matrices.
#' If the list contains weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge. .
#'
...
#'
#' ## Under H0
#' # Graphs[[1]] <- erdos.renyi.game(50, 0.5)
#' # Graphs[[2]] <- erdos.renyi.game(50, 0.5)
#' # Graphs[[3]] <- erdos.renyi.game(50, 0.5)
#' # result <- sp.anogva(Graphs, model, maxBoot = 300)
#' # result
#'
#' ## Under H1
#' # Graphs[[1]] <- erdos.renyi.game(50, 0.5)
#' # Graphs[[2]] <- erdos.renyi.game(50, 0.55)
#' # Graphs[[3]] <- erdos.renyi.game(50, 0.5)
#' # result <- sp.anogva(Graphs, model, maxBoot = 300)
#' # result
#'
#' @export
sp.anogva <- function(Graphs, model, maxBoot=500, spectra = NULL, eps = 0.01,
                      classic = FALSE) {
  numGraphs <- length(Graphs)
  Graphs <- graphList_to_adjList(Graphs)
  p.hat <- rep(-1,numGraphs)
  for(l in 1:numGraphs) {
    n <- as.character(ncol(Graphs[[l]]))
    if (methods::is(spectra,"list"))
      p.hat[l] <- graph.diag.param.estimator(Graphs[[l]], model, eps=eps,
                                             spectra = spectra[[n]],
                                             classic = classic)$p
    else p.hat[l] <- graph.diag.param.estimator(Graphs[[l]], model, eps=eps,
                                                spectra = spectra,
                                                classic = classic)$p
  }
  p.boot <- matrix(0, numGraphs, maxBoot)
  for (boot in 1:maxBoot) {
    g.boot <- list()
    for (l in 1:numGraphs) {
      n <- as.character(ncol(Graphs[[l]]))
      if (model == "ER") {
        g.boot[[l]] <- ER(ncol(Graphs[[l]]), p.hat[[l]])
      }
      if (model == "GRG") {
        g.boot[[l]] <- GRG(ncol(Graphs[[l]]), p.hat[[l]])
      }
      if (model == "WS") {
        g.boot[[l]] <- WS(ncol(Graphs[[l]]),p.hat[[l]],2)
      }
      if (model == "BA") {
        g.boot[[l]] <- BA(ncol(Graphs[[l]]), p.hat[[l]])
      }
      if(methods::is(spectra,"list"))
        p.boot[l,boot] <- graph.diag.param.estimator(g.boot[[l]], model, eps=eps,
                                                     spectra = spectra[[n]],
                                                     classic = classic)$p
      else p.boot[l,boot] <- graph.diag.param.estimator(g.boot[[l]], model, eps=eps,
                                                        spectra = spectra,
                                                        classic = classic)$p
    }
  }
...
#'
#' \code{graph.corr.test} tests for association between paired samples of graphs,
#' using Spearman's rho correlation coefficient.
#'
#' @param Graphs1 a list of (un)weighted graphs or a list of adjacency (symmetric) matrices.
#' If the list contains weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge.
#'
#' @param Graphs2 a list of (un)weighted graphs or a list of adjacency (symmetric) matrices.
#' If the list contains weighted graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge.
#'
#' @return
...
#'
#' for (i in 1:50) {
#'     Graphs1[[i]] <- erdos.renyi.game(50, p[i,1])
#'     Graphs2[[i]] <- erdos.renyi.game(50, p[i,2])
#' }
#'
#' graph.corr.test(Graphs1, Graphs2)
#'
#' @import stats
#' @import MASS
#' @export
graph.corr.test <- function(Graphs1, Graphs2) {
  
  x <- graphList_to_adjList(Graphs1)
  y <- graphList_to_adjList(Graphs2)
...
#' p <- (p - mi)/(ma-mi)
#' for (t in 1:100) {
#'     Graphs[[t]] <- erdos.renyi.game(100, p[t])
#' }
...
graph.acf <- function(Graphs, plot=TRUE) {
  Graphs <- graphList_to_adjList(Graphs)
  x.radius <- array(0, length(Graphs))
  for (t in 1:length(x)) {
    x.radius[t] <- eigen(Graphs[[t]], only.values=TRUE, symmetric=TRUE)$values[1]
  }
...
#' Graphs <- list()
#' for (i in 1:5) {
#'     Graphs[[i]] <- erdos.renyi.game(50, 0.5, type="gnp",directed = FALSE)
#' }
#' for (i in 6:10) {
#'     Graphs[[i]] <- watts.strogatz.game(1, 50, 8, 0.2)
#' }
#' for (i in 11:15) {
#'     Graphs[[i]] <- barabasi.game(50, power = 1,directed = FALSE)
#' }
#' graph.cluster(Graphs, 3)
#'
#' @import stats
#'
#' @export
graph.cluster <- function(Graphs, k, method="complete", bandwidth="Silverman") {
  
  f <- nSpectralDensities(Graphs, bandwidth=bandwidth)
  
  d <- matrix(0, length(Graphs), length(Graphs))
  for (i in 1:(length(Graphs)-1)) {
    f1 <- list("x"=f$x, "y"=f$densities[,i])
    for (j in (i+1) : length(Graphs)) {
...
#'     Graphs[[i]] <- barabasi.game(50, power = 1,directed = FALSE)
#' }
#' graph.mult.scaling(Graphs)
#'
#' @import graphics
#' @export
graph.mult.scaling <- function(Graphs, plot=TRUE, bandwidth="Silverman", type="n",
                               main="", ...) {
  
  f <- nSpectralDensities(Graphs, bandwidth=bandwidth)
  
  d <- matrix(0, length(Graphs), length(x))
  for (i in 1:(length(Graphs)-1)) {
    f1 <- list("x"=f$x, "y"=f$densities[,i])
    for (j in (i+1) : length(Graphs)) {
      f2 <- list("x"=f$x, "y"=f$densities[,j])
  ...
  if (is.null(names(Graphs)))
    names <- as.character(1:length(Graphs))
  else
    names <- names(Graphs)
...
#'
#' @param Graph1 the first undirected graph to be compared. Must be an igraph
#' object.
#'
#' @param Graph2 the second undirected graph to be compared. Must be an igraph
#' object.
#'
...
#' Graph1 <- sample_dot_product(lpvs)
#' Graph2 <- sample_dot_product(lpvs)
#' D <- tang.test(Graph1,Graph2, 5, printResult = TRUE)
#'
#' ## test under H1
...
#'
#' @export
tang.test <- function(Graph1, Graph2, dim, sigma = NULL, alpha = 0.05, bootstrap_sample=200,
                      printResult = FALSE){
  ...
  if(is.null(sigma)){
    sigma = tang.getSigma(Xhat1, Xhat2)
  }
...
#'
#' @param Graphs1 list of undirected graphs to be compared. Must be a list
#' of matrices or igraph objects.
#'
#' @param Graphs2 list of undirected graphs to be compared. Must be a list
#' of matrices or igraph objects.
#'
...
#' Graphs1 <- Graphs2 <- list()
#' for(i in 1:10){
#'   Graphs1[[i]] <- as.matrix(get.adjacency(erdos.renyi.game(50,0.6)))
#'   Graphs2[[i]] <- as.matrix(get.adjacency(erdos.renyi.game(50,0.6)))
#' }
#' D <- ghoshdastidar(Graphs1, Graphs2, printResult = TRUE)
#'
#' ## test for sets with more than one graph each under H1
#' Graphs1 <- Graphs2 <- list()
#' for(i in 1:10){
#'   Graphs1[[i]] <- as.matrix(get.adjacency(erdos.renyi.game(50,0.6)))
#'   Graphs2[[i]] <- as.matrix(get.adjacency(erdos.renyi.game(50,0.7)))
#' }
#' D <- ghoshdastidar(Graphs1, Graphs2, printResult = TRUE)
#'
#' ## test for sets with only one graph each under H0
#' Graphs1 <- Graphs2 <- list()
#' Graphs1[[1]] <- erdos.renyi.game(300, 0.6)
#' Graphs2[[1]] <- erdos.renyi.game(300, 0.6)
#' D <- ghoshdastidar(x, y, two.sample= TRUE, printResult = TRUE)
#'
#' ## test for sets with only one graph each under H1
#' Graphs1 <- Graphs2 <- list()
#' Graphs1[[1]] <- erdos.renyi.game(300, 0.6)
#' Graphs2[[1]] <- erdos.renyi.game(300, 0.7)
#' D <- ghoshdastidar(Graphs1, Graphs2, two.sample= TRUE, printResult = TRUE)
#' }
#'
#' @export
ghoshdastidar <- function(Graphs1, Graphs2, maxPer = 300, alpha = 0.05, two.sample = FALSE,
                          printResult = FALSE)
{
  Graphs1 <- graphList_to_adjList(Graphs1)
  Graphs2 <- graphList_to_adjList(Graphs2)
...
#' null hypothesis.
#'
#' @param Graphs1 the first iid sample of graphs to be compared. Must be a list of
#' igraph objects.
#'
#' @param Graphs2 the second iid sample of graphs to be compared. Must be a list of
#' igraph objects.
#'
#' @param maxPer integer indicating the number of bootstrap resamples (default
#' is 300).
...
#' Graphs1 <- Graphs2 <- list()
#' for(i in 1:10){
#'   Graphs1[[i]] <- erdos.renyi.game(50,0.5)
#'   Graphs2[[i]] <- erdos.renyi.game(50,0.5)
#' }
#' k <- cerqueira(Graphs1, Graphs2, printResult = TRUE)
#'
#' ## test under H1
#' Graphs1 <- Graphs2 <- list()
#' for(i in 1:10){
#'   Graphs1[[i]] <- erdos.renyi.game(50,0.5)
#'   Graphs2[[i]] <- erdos.renyi.game(50,0.6)
#' }
#' k <- cerqueira(a, b, printResult = TRUE)
#' }
#'
#' @export
cerqueira <- function(Graphs1, Graphs2, maxPer = 300, alpha = 0.05, printResult = FALSE)
{
  
  if(methods::is(Graphs1,"list") && methods::is(Graphs1[[1]],"igraph")){
    g <- cerqueira.transform(Graphs1)
  }
  else{
    stop("Parameter g must be an a list of igraph objects.")
  }
  
  if(methods::is(Graphs2,"list") && methods::is(Graphs2[[1]],"igraph")){
...
#' network.
#'
#' @param Graphs the undirected graphs to be compared. Must be a list of lists of
...
#' }
#' Graphs <- list(a,b)
#' k <- fraiman(Graphs, printResult = TRUE)
#'
#' ## test under H1
#' a <- b <- list()
#' for(i in 1:10){
...
#'
#' @export
fraiman <- function(Graphs, maxPer = 300, alpha = 0.05, printResult = FALSE){
  # transform and verify input
  if(methods::is(Graphs,'list') && methods::is(g[[1]],'list') && methods::is(g[[1]][[1]],'igraph')){
    Graphs <- graphList_to_adjList(Graphs)
  }
  if(!methods::is(Graphs,'list') || !methods::is(Graphs[[1]],'list') || !methods::is(Graphs[[1]][[1]],'matrix') || !methods::is(Graphs[[1]][[1]],'igraph'))
    stop(paste("You must pass 'Graphs' as a list of lists of igraphs or a list of lists of",
               "matrices."))
...
#'  }
#'  res <- graph.CEM(g, model="ER", num_clusters=2, max_iter=1, ncores=1)
#' @export
graph.CEM <- function(Graphs, model, num_clusters, max_iter = 10, ncores=1){
  ...
  Graphs <- graphList_to_adjList(Graphs)
  tipo <- model
  tau <- matrix(0, nrow = num_clusters, ncol = length(Graphs))
  kl <- matrix(0, nrow = num_clusters, ncol = length(Graphs))
  
  prevlik <- 0
  lik <- 1
  count <- 0
  prevlabels <- array(0, length(g))
  vertices <- nrow(Graphs[[1]])
  labels <- array(0, length(Graphs))
  g_GIC <- array(0, length(Graphs))
  ...
  eigenvalues <- foreach::foreach(j = 1:length(Graphs),.export = c("graph.eigen")) %dopar% {
    graph.eigen(Graphs[[j]])
  }
  
  p_graph <- array(0, length(Graphs))
  ...
  ret <- foreach::foreach(i = 1:length(Graphs),.export = c("graph.param.estimator",list_functions)) %dopar% {
    graph.param.estimator(Graphs[[i]], model=tipo, parameters=parameters, bandwidth="Sturges", eigenvalues=eigenvalues[[i]], eps=0.01)
  }
  #
  ...
  count <- 0
  while(!convergiu){
    kl <- foreach::foreach(i = 1:num_clusters, .combine=cbind,.export = list_functions) %:% foreach::foreach(j = 1:length(Graphs), .combine=c,.export = list_functions) %dopar% {
      GIC(Graphs[[j]], tipo, p[i], bandwidth = "Sturges", eigenvalues = eigenvalues[[j]], dist="KL" )
    }
    kl <- t(kl)
...
#'
#' @param Graphs a list of (un)weighted graphs or a list of adjacency (symmetric) matrices.
#' If the list contains graphs then the weights of the edges should be stored in the
#' "weight" attribute of the edge.
#'
...
#' Graphs <- list()
#' for(i in 1:5){
#'   Graphs[[i]] <- sample_gnp(30, p=0.2)
#' }
#' for(i in 6:10){
#'   Graphs[[i]] <- sample_gnp(30, p=0.5)
#' }
#' res <- graph.kmeans(Graphs, k=2, nstart=2)
#'
#' @export
graph.kmeans <- function(Graphs, k, nstart=2) {
  sil <- -1
  num.graphs <- length(Graphs)
  
  tmp <- nSpectralDensities(Graphs,bandwidth = "Sturges") # spectral_density(Graphs)
  
  spectral_densities <- tmp$densities#$spectral.density
  ...
  for (ns in 1:nstart) {
    ...
    while(converged == FALSE) {
      ...
      for (j in 1:k) {
        for (i in 1:512) {
          centroid[j,i] <- mean(spectral_densities[which(label==j),i])
        }
...
#'
#' \code{graph.fast.eigen.prob} returns the probability of an eigenvalue
...
#' z <- 0 + 0.01*1i
#' eigenval_prob <- -Im(graph.fast.eigen.prob(deg_prob,q_prob,all_k,z))
#' eigenval_prob
#'
#' @export
graph.fast.eigen.prob <- function(deg_prob,q_prob,all_k,z,n_iter = 5000){
  h_z   <- 0 + 0i
  eps <- 1e-7
  all_k_mo <- all_k - 1
  while(n_iter > 0){
...
# create a function called graph.spectral.density(): method = "exact"|"degree" -> returns the spectral density
# create a function called graph.param.estimator(): method = "exact"|degree

#' Degree-based spectral density
#'
#' \code{graph.fast.spectral.density} returns the degree-based spectral density in
#' the interval <\code{from},\code{to}> by using npoints discretization points.
#'
...
#' density <- graph.fast.spectral.density(Graph = G, npoints = 80, numCores = 1)
#' density
#'
#' @export
graph.fast.spectral.density <- function(Graph, from = NULL, to = NULL, npoints = 1024,
  ...
  if(is.null(from) || is.null(to)){
    A <- igraph::get.adjacency(Graph,type = "both")
  }
  ...
  doParallel::registerDoParallel(cl)
  i <- NULL
  y <- foreach::foreach(i=1:length(x),.combine = c,.export = c("graph.fast.eigen.prob")) %dopar% {
    z <- x[i] + 0.01*1i
    -Im(graph.fast.eigen.prob(deg_prob,q_prob,all_k,z))
  }
  # close cluster
  parallel::stopCluster(cl)
...
#'
#' \code{graph.fast.param.estimator} estimates the parameter of the complex
...
#'
#' @param parameters list with elements 'lo' and 'hi' defining an interval search 
#' of the parameter.
#' The 'graph.fast.param.estimator' will return the element of 'parameter' 
#' that minimizes the L1 distance. If the user does not provide the argument 
#' 'parameters', and 'model' is an array, then the values considered for the 
#' parameter estimation are the rownames converted to a numeric vector.
#' If 'model' is a string, then default values are used for the predefined 
#' models ("ER", "GRG", "KR", "WS", and "BA"). 
#' The default 'parameter' argument corresponds to a sequence from:
#'
#' 0 to 1 with step 'eps' for the "ER" model (Erdos-Renyi random graph), in
#' which the parameter corresponds to the probability to connect a pair of
#' vertices;
...
#' parameters = list(lo = 0,hi = 0.5)
#' estimated.parameter <- graph.fast.param.estimator(G, "WS",parameters,eps = 1e-1, npoints = 10,
#'                                                   numCores = 1)
#' estimated.parameter
...
#' #}
#' # Obtain the parameter of the WS model
#' parameters = list(lo = 0.0, hi = 1.0)
#' #estimated.parameter <- graph.fast.param.estimator(G, fun_WS, parameters = parameters,
#' #                                                   npoints = 100, numCores = 2)
#' #estimated.parameter
#'
#' @export
graph.fast.param.estimator <- function(Graph, model,  parameters=NULL,
  ...
  if(methods::is(model,"function") && (is.null(parameters$lo) || is.null(parameters$hi))){
  ...
  observed_density <- graph.fast.spectral.density(Graph, from = from, to = to,
                                                  npoints = npoints,
                                                  numCores = numCores)
  # Make ternary search
  dist_to_1 <- NULL
  ...
  while(abs(lo - hi) > eps && lo < hi){
    ...
    density1 <- graph.fast.spectral.density(Graph1, from = from, to = to,
                                            npoints = npoints,
                                            numCores = numCores)
    ...
    density2 <- graph.fast.spectral.density(Graph2, from = from, to = to,
                                            npoints = npoints,
                                            numCores = numCores)
    # Free memory allocated by Graph2
    rm(Graph2)
    # Reduce the interval search
    dist_to_1 <- trapezoidSum(observed_density$x,abs(observed_density$y - density1$y))
    dist_to_2 <- trapezoidSum(observed_density$x,abs(observed_density$y - density2$y))
...
gaussianDensity <- function(x, from=NULL, to=NULL, bandwidth="Silverman",
  ...
  if (bw <= 0)
    stop("Parameter bw (bandwidth) should be greater than zero.")
  if (is.null(from) || is.null(to))
...
graphList_to_adjList <- function(Graphs){
  if(methods::is(Graph,'igraph')){ return(graphToAdj(Graph)) }
  else if(methods::is(Graph,'list') && methods::is(Graph[[1]],'igraph')){
    d <- lapply(Graph, graphList_to_adjList)
    return(d)
  }
  else if(methods::is(Graph,'list') && methods::is(Graph[[1]],'list')){
    d <- lapply(Graph, graphList_to_adjList)
    return(d)
  }
}
...
# same points
nSpectralDensities <- function (Graphs, from=NULL, to=NULL,
                                bandwidth="Silverman") {
  npoints <- 1024
  ngraphs <- length(Graphs)
  ns <- unlist(lapply(Graphs, ncol))
  #n <- ncol(adjacencyMatrices[[1]])
  spectra <- matrix(NA, max(ns), ngraphs)
  for (i in 1:ngraphs) {
    A <- graphToAdj(Graphs[[i]])
    n <- ncol(A)
  ...
  for (i in 1:ngraphs) {
    ...
    f <- gaussianDensity(spectra[1:n,i], bandwidth=bandwidth,
                         ...
                         npoints=npoints)
    # normalize area
    densities[,i] <- f$y ## normalize to AUC == 1
    x <- f$x
  }
  return(list("x"=x, "densities"=densities))
}

# Check if object is igraph or an adjacency matrix. If it is, then return the adjacency matrix,
# otherwise raise an error

graphToAdj <- function(Graph){
  A <- NULL
  if(methods::is(Graph,"igraph")){
    if(is.null(E(Graph)$weight)){
      A <- igraph::get.adjacency(Graph,type = "both")
    } else {
      A <- igraph::get.adjacency(Graph,type = "both",attr="weight")
    }
  } else if(methods::is(Graph,"matrix") || methods::is(Graph,"dgCMatrix")){
    if(nrow(Graph) == ncol(Graph)){
      A <- Graph
    } else{
      stop("Input object 'Graph' should be either an igraph object or a square Matrix.")
    }
  } else {
    stop("Input object 'Graph' should be either an igraph object or a square Matrix.")
  }
...
# Estimates the spectral density of a graph model using the extact method or degree-based method

graph.model.spectral.density <- function(fun,n,p,from = NULL, to = NULL,method = "diag",...){
  parameters = list(Graph = Graph,from = from,to = to,npoints = npoints)
  other_parameters = list(...)
  all_parameters = c(parameters,other_parameters)
...
# Estimates the spectral density of a graph model using the exact spectral density method
graph.fast.model.spectral.density <- function(fun,n,p,from = NULL, to = NULL, npoints = 1024,numCores = 1){
  Graph <- fun(n,p)
  model_density <- graph.fast.spectral.density(Graph = Graph,from = from,to = to,npoints = npoints,numCores = numCores)
  return (model_density)
}

# Estimates the spectral density of a graph model using the exact spectral density method
graph.diag.model.spectral.density <- function(fun, n, p, ngraphs=100, from=NULL, to=NULL,
                                              bandwidth="Silverman", npoints=1024) {
  spectra <- matrix(NA, n, ngraphs)
  for (i in 1:ngraphs) {
    eigenvalues <- graph.eigen(fun(n, p))
...
cerqueira.test <- function(Graph1, Graph2)
{
  wstat <- sum(abs(colMeans(Graph1)-colMeans(Graph2)))
  return(wstat)
}
...
# Auxiliary for Fraiman method. Intends to speed calculations using R builtins.
fraiman.calcM <- function(x){ mapply(f.div, mapply(f.add, x, SIMPLIFY = F), SIMPLIFY = F)}
...
# obtain the eigenvalues of a Graph and scale it
graph.eigen <- function(Graph){
  A <- graphToAdj(Graph)
  eigenvalues <- eigen(A, only.values = TRUE,symmetric=TRUE)$values

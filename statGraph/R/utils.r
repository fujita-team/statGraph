...
gaussianDensity <- function(x, from=NULL, to=NULL, bandwidth="Silverman",
  ...
  return(list("x"=f$x, "y"=f$y/area,"from" = min(f$x),"to" = max(f$x)))
}
...
# Returns a distance matrix between graphs, given a distance measure
distance_matrix <- function(Graphs,dist = "JS") {
  nGraphs <- length(Graphs)
  distance <- matrix(0, nGraphs, nGraphs)
  for (i in 1:(nGraphs-1)) {
    for (j in (i+1):nGraphs) {
      distance[i,j] <- distance[j,i] <- distance(Graphs[[i]]$density,Graphs[[j]]$density,dist = dist)
    }
  }
  return(distance)
}

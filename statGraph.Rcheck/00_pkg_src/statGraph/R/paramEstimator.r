...
graph.param.estimator <- function(Graph, model, interval = NULL, eps=0.01,search = "grid",...) {
  ...
  if(methods::is(search_interval,"atomicVector")){
    ...
    for (p in search_interval){
      curr_GIC <- GIC(Graph = Graph, model = model, p = p, ...)
      if (curr_GIC < min_GIC) {
        min_GIC <- curr_GIC
        pmin <- p
      }
    ...
    for(interval in search_interval)
    {
      ...
      while(TRUE)
      {
        if(abs(lo - hi) < eps || lo > hi)
        {
          ...
          curr_GIC <- GIC(Graph = Graph , model = model, p = pmin,...)
          break
        }
        ...
        GIC_1 <- GIC(Graph = Graph, model, p = leftThird,...)
        GIC_2 <- GIC(Graph = Graph, model, p = rightThird,...)

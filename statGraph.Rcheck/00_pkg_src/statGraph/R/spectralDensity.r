...
#' \itemize{
#' \item{\code{method='diag'}:}{ then the parameter \code{bandwidth} can be used.
...
#' \item{\code{method='fast'}:}{ then
#' the parameter \code{numCores} can be used. This parameter
#' specifies the number of cores (default \code{1}) to use for parallelization.}
#' }
#'
#'
#' @return Returns the spectral density of the graph.
#'
...
graph.spectral.density <- function(Graph,method = "diag",...){
  ...
  if(is.null(Graph$density)){
    ...
    if(method == "diag") return (graph.diag.spectral.density(Graph = Graph,from = density_parameters$from,to = density_parameters$to,bandwidth = density_parameters$bandwidth,npoints = density_parameters$npoints))
    if(method == "fast") return (graph.fast.spectral.density(Graph = Graph,from = density_parameters$from,to = density_parameters$to,npoints = density_parameters$npoints,numCores = density_parameters$numCores))
